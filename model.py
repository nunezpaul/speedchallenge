...
class DeepVO(object):
    ...
    def setup_model(self, train_data, categorical_weight, speed_weight):
        ...
        loss_weights = {'category': categorical_weight, 'speed': speed_weight}
    ...
    def cnn(self, model_input):
        vgg = VGG16(weights='imagenet',
                       include_top=False,
                       input_shape=model_input.shape.as_list()[1:])
        vgg.trainable = False
        output = vgg(model_input)
    ...
    def predict(self, data, save_dir):
        ...
        for step in range(data.len // data.batch_size + 1):
            ...
            prediction = DataFrame(prediction_logits[0])
